...
export class PolygonTool extends Tool {
  ...
  constructor(
    svg: SVGSVGElement,
    onComplete: (shape: PolygonShape) => void,
  ...
  override handleMouseDown(point: Point, event: PointerEvent): void {
    if (event.button === 0) {
      // Left click only
      const clamped = (this.constructor as typeof Tool).clampToImageBounds(
        point,
        Tool.imageBounds
      );
  ...
  private completeShape(): void {
    if (
      ...
      this.currentShape &&
      this.points.length >= this.minPoints
    ) {
      ...
      this.points = this.points.map((p) =>
        (this.constructor as typeof Tool).clampToImageBounds(
          p,
          Tool.imageBounds
        )
      );
